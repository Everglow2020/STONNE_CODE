...
Stonne::Stonne(Config stonne_cfg) {
	...
	case MAERI_DENSE_WORKLOAD:
	    ...
	    break;
	/*case MAERI_DENSEGEMM_WORKLOAD://新增
	    this->mem = new  DenseSDMemory(0, "DenseSDMemory", stonne_cfg, this->outputLTConnection);
	    break;*/
...
void Stonne::loadGEMM(std::string layer_name, unsigned int N, unsigned int K, unsigned int M, address_t MK_matrix, address_t KN_matrix, metadata_address_t MK_metadata, metadata_address_t KN_metadata, address_t output_matrix, metadata_address_t output_metadata, Dataflow dataflow) {
    ...
    //filter_matrix = MK
    //loadDNNLayer目前不用改
    loadDNNLayer(GEMM, layer_name, 1, K, 1, M, 1, 1, N, K, 1, MK_matrix, KN_matrix, output_matrix, dataflow);//MK_matrix, KN_matrix是压缩后的稀疏矩阵
    std::cout << "Loading a GEMM into STONNE" << std::endl;
    this->mem->setSparseMetadata(MK_metadata, KN_metadata, output_metadata); //MK_metadata, KN_metadata是bitmap
    std::cout << "Loading metadata" << std::endl;
}

void Stonne::loadGEMM(std::string layer_name, unsigned int N, unsigned int K, unsigned int M, address_t MK_matrix, address_t KN_matrix, metadata_address_t MK_rowptr,metadata_address_t MK_rowindex, metadata_address_t MK_colindex, metadata_address_t KN_rowptr,metadata_address_t KN_rowindex, metadata_address_t KN_colindex, metadata_address_t MK_metadata, metadata_address_t KN_metadata,address_t output_matrix, metadata_address_t output_rowptr,metadata_address_t output_rowindex,metadata_address_t output_colindex,metadata_address_t output_metadata,Dataflow dataflow) {
    loadDNNLayer(GEMM, layer_name, 1, K, 1, M, 1, 1, N, K, 1, MK_matrix, KN_matrix, output_matrix, dataflow);//MK_matrix, KN_matrix是压缩后的稀疏矩阵
    std::cout << "Loading a GEMM into STONNE" << std::endl;
    this->mem->setSparseMetadata(MK_metadata, KN_metadata, output_metadata,MK_rowptr,MK_rowindex, MK_colindex, KN_rowptr,KN_rowindex,KN_colindex, output_rowptr,output_rowindex,output_colindex); 
    std::cout << "Loading metadata" << std::endl;
}
...
void Stonne::cycle() {//1.mem 2.collectionBus 3.asnet & lt 4.msnet 5.dsnet
