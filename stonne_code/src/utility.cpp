...
MemoryController_t get_type_memory_controller_type(std::string memory_controller_type) {
		...
		else if(memory_controller_type=="MAERI_DENSEGEMM_WORKLOAD") {//新增
            return MAERI_DENSEGEMM_WORKLOAD;
        }
...
unsigned int* calculateOrdering (float* matrix, unsigned int rows, unsigned int cols, GENERATION_TYPE gen_type, int num_ms) {
    ...
    unsigned int* pointer_table;
    unsigned int* size_rows;
    if(gen_type==GEN_BY_ROWS) {//每行的非零元素只是为3
        pointer_table = new unsigned int[rows];
        size_rows = new unsigned int [rows];
    ...
    unsigned int n_rows_selected = 0;//n_rows_selected也代表着不合适的行(列)数，即非零元素多于ms数的行数
  
    //First the ones that does not fit 
    for(int i=0; i<dim_table; i++) {//把非零元素多于ms数的行(列)调换到前面去
        if(size_rows[i] > num_ms) {
            int temp = size_rows[i];//互换pointer_table和size_rows值
    ...
    while(n_rows_selected < dim_table) {
        row_found = false;
		//这个for循环是为了
		//从符合条件的行(非零元素个数<=剩余ms数)中选取非零元素最多的一行排到前面去

		for(int i=n_rows_selected; i < dim_table; i++) {//从第一个非零元素少于ms数的行(列)开始遍历
		    if((size_rows[i] + n_ms_used) <= num_ms) {//若该行非零元素个数<=剩余ms数
				if(row_found == false) {
		              row_found = true;
		              greater_row = i;//标记这一行，为greater_row
				}
				else { //If it is true 
    ...
    delete[] size_rows;
    return pointer_table;//pointer_table为长度M(N),前面一部分全是非零元素多于ms数的，后面部分就像打补丁一样逐渐填满（比如ms数为64，后面部分有5行，非零元素个数分别为50、40、18、12、2，冒泡排序后为50、12、2、40、18）
}
