...
int matrix_to_csr(int m,int n,float *Mat,float* &value,unsigned int* & rowptr,unsigned int* & rowindex,unsigned int* & colindex){
   int i,j;
   int a=0;
   for(i=0;i<m;i++)
      for(j=0;j<n;j++)
          if(Mat[i*n+j]!=0)
              a++;
   value=new float[a];
   colindex=new unsigned int[a];
   rowindex=new unsigned int[a];
...
bool runSparseGEMMCommand(int argc, char *argv[]) {
    ...
    //Creating MK matrix
    //float* MK_dense_matrix_no_organized = generateMatrixDense(M, K, MK_sparsity);//根据稀疏度生成一个矩阵，实际上是一个一维的长度为M*K的序列
    float* MK_dense_matrix = new float[M*K];
    ///*
    float* MK_dense_matrix_no_organized = new float[M*K];
    /*MK_dense_matrix_no_organized[0]=0;
    MK_dense_matrix_no_organized[1]=1;
    MK_dense_matrix_no_organized[2]=2;
    MK_dense_matrix_no_organized[3]=3;
    MK_dense_matrix_no_organized[4]=4;
    MK_dense_matrix_no_organized[5]=5;
    MK_dense_matrix_no_organized[6]=6;
    MK_dense_matrix_no_organized[7]=0;
    MK_dense_matrix_no_organized[8]=7;
    MK_dense_matrix_no_organized[9]=8;
    MK_dense_matrix_no_organized[10]=9;
    MK_dense_matrix_no_organized[11]=0;*/
    MK_dense_matrix_no_organized[0]=0;
    MK_dense_matrix_no_organized[1]=1;
    MK_dense_matrix_no_organized[2]=2;
    MK_dense_matrix_no_organized[3]=3;
    ...
    float* KN_dense_matrix = new float[K*N];
	///*
	float* KN_dense_matrix_no_organized = new float[K*N];
	KN_dense_matrix_no_organized[0]=1;
	KN_dense_matrix_no_organized[1]=0;
	KN_dense_matrix_no_organized[2]=6;
	KN_dense_matrix_no_organized[3]=0;
	KN_dense_matrix_no_organized[4]=2;
	KN_dense_matrix_no_organized[5]=4;
	KN_dense_matrix_no_organized[6]=0;
	KN_dense_matrix_no_organized[7]=0;
	KN_dense_matrix_no_organized[8]=0;
	KN_dense_matrix_no_organized[9]=5;
	KN_dense_matrix_no_organized[10]=0;
	KN_dense_matrix_no_organized[11]=0;
	KN_dense_matrix_no_organized[12]=3;
	KN_dense_matrix_no_organized[13]=0;
	KN_dense_matrix_no_organized[14]=7;
	KN_dense_matrix_no_organized[15]=0;
	//*/
	cout << "MK:" ;
    for(int i=0; i<M*K; i++) {
        MK_dense_matrix[i]=MK_dense_matrix_no_organized[i];
		cout << MK_dense_matrix[i] << " ";//新增
    }
	cout << endl;
	cout << "KN:" ;
    for(int i=0; i<K*N; i++) {
        KN_dense_matrix[i]=KN_dense_matrix_no_organized[i];
		cout << KN_dense_matrix[i] << " ";//新增
    }
	cout << endl;

    //See if it is necessary to reorganize
    //为什么要重新组织？只对STA矩阵重新组织
    ...
    if(optimize & (dataflow == MK_STA_KN_STR)) {
        ...
        organizeMatrix (MK_dense_matrix, M, K, order_table, GEN_BY_ROWS);//按照order_table对行的顺序进行重排列（比如矩阵为【【1、2、3】，【4、5、6】，【7、8、9】】，order_table为[3、2、1]，则重新组织后矩阵变为【【7、8、9】，【4、5、6】，【1、2、3】】）
    }
        ...
        organizeMatrix (KN_dense_matrix, K, N, order_table, GEN_BY_COLS);//同上，只不过换成了列变换
    }
	...
	//Generating CSR
	unsigned int* MK_rowptr = NULL;
	unsigned int* MK_rowindex = NULL;
	unsigned int* MK_colindex = NULL;
	unsigned int* KN_rowptr = NULL;
	unsigned int* KN_rowindex = NULL;
	unsigned int* KN_colindex = NULL;
	unsigned int* acc_rowptr = new unsigned int[M*N];
	unsigned int* acc_rowindex = new unsigned int[M*N];
	unsigned int* acc_colindex = new unsigned int[M*N];
	float* MK_value = NULL;
	float* KN_value = NULL;
	int a = matrix_to_csr(M, K, MK_dense_matrix, MK_value, MK_rowptr, MK_rowindex, MK_colindex);
	int b = matrix_to_csr(K, N, KN_dense_matrix, KN_value, KN_rowptr, KN_rowindex, KN_colindex);
	cout << "MK_bitmap:" << endl;
	for(int i=0; i<M; i++) {
		for(int j=0; j<K; j++){
			cout << MK_bitmap[i*K+j] << " ";//新增
		}
		cout << endl;
    }
	cout << "MK_rowptr:" << endl;
	...
	cout << "MK_rowindex:" << endl;
	for(int i=0;i<a;i++)
    {
      cout << MK_rowindex[i] << " ";
    }cout << endl;
	cout << "MK_colindex:" << endl;
	for(int i=0;i<a;i++)
    {
      cout << MK_colindex[i] << " ";
    }cout << endl;
	...
	//Generating matrix C index
	//vector* matrixC = 
	//TEST
	///*
	for(int i=0;i<9;i++)
		cout << MK_sparse_matrix[i] << " ";
	cout << endl;
	for(int i=0;i<7;i++)
		cout << KN_sparse_matrix[i] << " ";
	cout << endl;
	//*/
    //Running STONNE
    Stonne* stonne_instance = new Stonne(stonne_cfg); //Creating instance of the simulator
    ...
    stonne_instance->loadGEMM(layer_name, N, K, M, MK_sparse_matrix, KN_sparse_matrix, MK_rowptr,MK_rowindex,MK_colindex, KN_rowptr,KN_rowindex,KN_colindex, MK_bitmap, KN_bitmap, acc_output, acc_rowptr,acc_rowindex,acc_colindex,acc_bitmap, dataflow );
    stonne_instance->run(); //Running the simulator
    if(optimize && (dataflow==MK_STA_KN_STR)) {
