...
/*从根到叶子遍历，从左到右构建结构。对于每个节点，它创建了一个ASwitch对象，并将其添加到aswitchtable中。它还创建了输入连接和输出连接，并将它们添加到inputconnectiontable和outputconnectiontable中。如果节点是第一个或最后一个，则它是单个减少开关。如果节点是偶数节点，则它需要一个转发连接。
它还创建了一个累加缓冲区，如果启用了累加缓冲区，则在析构函数中删除它。
