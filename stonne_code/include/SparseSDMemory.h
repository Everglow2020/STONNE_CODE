...
private:
	...
	//新增
	metadata_address_t STA_rowptr;
	metadata_address_t STA_rowindex;
	metadata_address_t STA_colindex;
	metadata_address_t STR_rowptr;
	metadata_address_t STR_rowindex;
	metadata_address_t STR_colindex;
	std::vector<unsigned int> needB;
	std::vector<Fifo*> needB_fifos;
    ...
    unsigned int sta_current_index_matrix; //Index to next element in the sparse matrix (sta矩阵已被分配完的元素的索引（当前最后一个被分配的）)
...
public:
    ...
    void setSparseMetadata(metadata_address_t MK_metadata, metadata_address_t KN_metadata, metadata_address_t output_metadata,metadata_address_t MK_rowptr,metadata_address_t MK_rowindex,metadata_address_t MK_colindex,metadata_address_t KN_rowptr,metadata_address_t KN_rowindex,metadata_address_t KN_colindex,metadata_address_t output_rowptr,metadata_address_t output_rowindex,metadata_address_t output_colindex); // Supported by this controller
