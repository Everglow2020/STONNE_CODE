...
private:
    ...
    unsigned int row_index;//新增
	unsigned int col_index;//新增
...
public:
	...
	DataPackage(size_t size_package, data_t data, operand_t data_type, id_t source,traffic_t traffic_type, unsigned int unicast_dest,unsigned int row_index,unsigned int col_index);
    ...
    DataPackage(size_t size_package, data_t data, operand_t data_type, id_t source,traffic_t traffic_type, bool* dests, unsigned int n_dests,unsigned int row_index,unsigned int col_index);
    //Broadcast package
    //Needs nothing. Just indicates is the type broadcast

    //ART Package constructor (only one package for this type)   Psum package
	...
	DataPackage(size_t size_package, data_t data, operand_t data_type, id_t source, unsigned int VN, adderoperation_t operation_mode,unsigned int row_index,unsigned int col_index);
    ~DataPackage();
