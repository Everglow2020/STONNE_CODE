...
//Created by LuoShengbai on 10/07/2023
#ifndef __DenseSDMEMORY__H__
#define __DenseSDMEMORY__H__
...
public:
    ...
    unsigned int base_K;  //N
    unsigned int base_X;  //M
    ...
    VNAT_Register1(unsigned int VN, unsigned int addr, unsigned int K, unsigned int X, 
    unsigned int iter_S, unsigned int iter_K, unsigned int iter_X, DNNLayer* dnn_layer, Tile* current_tile);
    void update(); //Update variables to the next cycle 
    unsigned int get_address();
    
};

class DenseSDMemory : public MemoryController {
private:
   ...
   VNAT_Register1** VNAT;  //VNAT with as many registers as VN configured in the accelerator
...
public:
    DenseSDMemory(id_t id, std::string name, Config stonne_cfg, Connection* write_connection);
    ~DenseSDMemory();
